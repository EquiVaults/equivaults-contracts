...
contract EquiVaultTest is Test {
    ...
    function setUp() public {
        ...
        vault = new EquiVault(usdc, registry, manager, assets(), weights(), 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
    }
    ...
    function testLowDecimalExitNeverRoundsFeeAboveRealizedGain() public {
        ...
        vm.prank(admin);
        registry.registerAsset(address(indivisible), primaryA, fallbackA, address(route), MAX_PRICE_AGE);
        ...
        EquiVault lowDecimals = new EquiVault(
            ...
            EquiVault.TimelockMode.Immutable, 0, 0, 0
        );
    ...
    function testConstructorRejectsInvalidParameters() public {
        ...
        new EquiVault(usdc, registry, manager, a, w, 2_001, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);

        vm.expectRevert(abi.encodeWithSelector(EquiVault.InvalidSlippage.selector, uint16(3_001)));
        new EquiVault(usdc, registry, manager, a, w, 1_000, 3_001, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);

        vm.expectRevert(abi.encodeWithSelector(EquiVault.InvalidAddress.selector));
        new EquiVault(usdc, registry, address(0), a, w, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);

        vm.expectRevert(abi.encodeWithSelector(EquiVault.InvalidBasketSize.selector, uint256(0)));
        new EquiVault(usdc, registry, manager, new address[](0), new uint16[](0), 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, six, sixW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, one, new uint16[](0), 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, two, lowW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, one, partialW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, dupAssets, dupW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, withUsdc, dupW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
        ...
        new EquiVault(usdc, registry, manager, unknown, fullW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
    }
    ...
    function testDualOracleFailurePausesOnlyAffectedVault() public {
        ...
        bW[0] = 10_000;
        EquiVault vault2 = new EquiVault(usdc, registry, manager, bOnly, bW, 1_000, 100, EquiVault.TimelockMode.Delayed, 1 days, 0, 0);
